...
sys.path.append('../../../')
from reservoir_acme.tf.networks import base
from reservoir_acme.tf.networks import duelling
from acme_time.acme.tf.networks import embedding
from reservoir_acme.tf.networks import policy_value
from reservoir_acme.tf.networks import vision
...
class AtariTorsoSmall(base.Module):
    """Simple convolutional stack commonly used for Atari."""

    def __init__(self):
        super().__init__(name='atari_torso_small')
        self._network = snt.Sequential([
            snt.Conv2D(32, [8, 8], [4, 4]),
            tf.nn.relu,
            snt.Conv2D(64, [4, 4], [2, 2]),
...
class DQNAtariNetwork(base.Module):
    ...
    def __init__(self, num_actions: int):
        ...
        self._network = snt.Sequential([
            AtariTorso(),
            snt.Linear(1000), tf.nn.tanh, snt.Linear(200), tf.nn.tanh, snt.Linear(100),
...
class DQNTimeAtariNetwork(base.Module):
    """A feed-forward network for use with Ape-X DQN.
    ...
    def __init__(self, num_actions: int):
        super().__init__(name='dqn_atari_network')
        self._features = snt.Sequential([AtariTorso(), snt.Linear(1000),
                                         tf.nn.tanh, snt.Linear(200), tf.nn.tanh, snt.Linear(100)])
        self._network = snt.Sequential([
            duelling.DuellingMLP(num_actions, hidden_sizes=[512]),
        ])

    def __call__(self, inputs: Images) -> QValues:
        return self._network(self._features(inputs))

    def get_feature(self, inputs: Images):
        return self._features(inputs)


class DFADQNAtari(base.Module):

    def __init__(self, num_of_actions: int):
        super().__init__(name='dfa_dqn_atari')
        image = [
            snt.Conv2D(32, [8, 8], [4, 4]),
            tf.nn.relu,
            snt.Conv2D(64, [4, 4], [2, 2]),
            ...
            tf.nn.relu,
            snt.Flatten()]
        image_layers = ['cnn', 'relu', 'cnn', 'relu', 'cnn', 'relu', 'flatten']
        atari_net = [snt.Linear(200), tf.nn.tanh, snt.Linear(100), tf.nn.tanh, snt.Linear(num_of_actions)]
        atari_layers = ['linear', 'tanh', 'linear', 'tanh', 'linear']
        self.network = snt.Sequential(image + atari_net)
        self.B = []
        self.output_size = num_of_actions
        self.layers = image_layers + atari_layers
        # logits = self.network(tf.random.normal([1, 256]))
        for i, name in enumerate(self.layers):
            layer = self.network._layers[i]
            if name == 'cnn':
                _shape = (layer.kernel_shape[0], layer.kernel_shape[1], 3, layer.output_channels)
                self.B.append(np.random.uniform(-1, 1, size=_shape))
            elif name == 'linear':
                self.B.append(np.random.uniform(-1, 1, size=(num_of_actions, layer.output_size)))
            else:
                self.B.append(None)
    ...
    def __call__(self, inputs: Images) -> QValues:
        ...
        for i in self.network._layers:
            layer_inputs.append(X)
            X = i(X)
            layer_outputs.append(X)
        X.layer_inputs = layer_inputs
        X.layer_outputs = layer_outputs
        return X


class DQN(base.Module):

    def __init__(self, num_of_actions: int):
        super().__init__(name='dqn')
        atari_net = [snt.Linear(1000), tf.nn.tanh, snt.Linear(500), tf.nn.tanh, snt.Linear(300), tf.nn.tanh,
                     snt.Linear(100), tf.nn.tanh, snt.Linear(num_of_actions)]
        self._network = snt.Sequential(
            [snt.Flatten()] + atari_net)
...
class DFADQN(base.Module):
    ...
    def __init__(self, num_of_actions: int):
        ...
        self.B = [None]
        self.output_size = num_of_actions
        self.layers = ['flatten', 'linear', 'tanh', 'linear', 'tanh', 'linear', 'tanh', 'linear', 'tanh', 'linear']
        for i in atari_net:
            try:
                self.B.append(np.random.uniform(-0.5, 0.5, size=(num_of_actions, i.output_size)))
            except:
                self.B.append(None)
...
class LSTMNetwork(base.RNNCore):

    def __init__(self, num_actions: int):
        super().__init__(name='lstmnetwork')
        self._lstm = snt.LSTM(512)
        self._head = duelling.DuellingMLP(num_actions, hidden_sizes=[512, 256, 100])

    def __call__(
            self, inputs: observation_action_reward.OAR,
            state: snt.LSTMState) -> Tuple[Tuple[Logits, Value], snt.LSTMState]:
        embeddings, new_state = self._lstm(inputs, state)
    ...
    def unroll(
        ...
        # embeddings = snt.BatchApply(self._lstm)(inputs)  # [T, B, D+A+1]
        embeddings, new_state = snt.static_unroll(self._lstm, inputs, state,
                                                  sequence_length)
...
class R2D2Network(base.RNNCore):
    ...
    def __call__(self, inputs: observation_action_reward.OAR,
                 state: snt.LSTMState) -> Tuple[QValues, snt.LSTMState]:
        embeddings, new_state = self._core(inputs, state)
    ...
    def unroll(
        ...
        """Efficient unroll that applies embeddings, MLP, & convnet in one pass."""
        embeddings, new_state = snt.static_unroll(self._core, inputs, state,
                                                  sequence_length)
        action_values = snt.BatchApply(self._head)(embeddings)
...
class R2D2SmallAtariNetwork(base.RNNCore):
    ...
    def __init__(self, num_actions: int):
        super().__init__(name='r2d2_small_atari_network')
        self._embed = embedding.OAREmbedding(
            torso=AtariTorsoSmall(), num_actions=num_actions)
        self._core = snt.LSTM(512)
        self._head = duelling.DuellingMLP(num_actions, hidden_sizes=[512])
        self.features = None
